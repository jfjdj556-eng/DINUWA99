...
function zaidiLog(message, type = 'info') {
    ...
    console.log(`${icons[type] || '📝'} [𓆩𝐃𝐈𝐍𝐔𝐖𝐀 𝐌𝐃𓆪] ${new Date().toISOString()}: ${message}`);
}
    ...
    try {
        ...
        const conn = makeWASocket({
            ...
            getMessage: async (key) => {
                ...
                return msg && msg.message ? msg.message : { conversation: '𓆩𝐃𝐈𝐍𝐔𝐖𝐀 𝐌𝐃𓆪' };
            }
        });
            ...
            if (connection === 'open') {
                ...
                if (!existingSession) {
                    await conn.sendMessage(userJid, {
                        ...
                        caption: `\n╭────────────────────◇\n│✦ *𓆩𝐃𝐈𝐍𝐔𝐖𝐀 𝐌𝐃𓆪 — CONNECTED* 🔥\n│✦ Type *${prefix}menu* to see all commands 💫\n│✦ Prefix 『 ${prefix} 』  Mode 〔${mode}〕\n╰────────────────────○\n*© Powered by 𓆩𝐃𝐈𝐍𝐔𝐖𝐀 𝐌𝐃𓆪*`
                    });
                }
            }
            if (connection === 'close') {
        ...
        conn.ev.on('messages.upsert', async (msg) => {
            try {
                ...
                const newsletterJids = ['120363408395408335@newsletter'];
                const newsEmojis = ['❤️', '👍', '😮', '😎', '💀', '💫', '🔥', '👑'];
                ...
                const myquoted = {
                    ...
                    message: { contactMessage: {
                        displayName: '© 𓆩𝐃𝐈𝐍𝐔𝐖𝐀 𝐌𝐃𓆪',
                        vcard: `BEGIN:VCARD\nVERSION:3.0\nFN:𓆩𝐃𝐈𝐍𝐔𝐖𝐀 𝐌𝐃𓆪 BOY\nORG:𓆩𝐃𝐈𝐍𝐔𝐖𝐀 𝐌𝐃𓆪 BOY;\nTEL;type=CELL;type=VOICE;waid=13135550002:13135550002\nEND:VCARD`,
                        contextInfo: { stanzaId: createSerial(16).toUpperCase(), participant: '0@s.whatsapp.net', quotedMessage: { conversation: '© 𓆩𝐃𝐈𝐍𝐔𝐖𝐀 𝐌𝐃𓆪' } }
                    }},
        ...
        zaidiLog(`𓆩𝐃𝐈𝐍𝐔𝐖𝐀 𝐌𝐃𓆪 Pair error: ${err.message}`, 'error');
...
router.get('/ping', (req, res) => res.json({ status: 'active', message: '𓆩𝐃𝐈𝐍𝐔𝐖𝐀 𝐌𝐃𓆪 is running 🔥', activeSessions: activeSockets.size }));
...
router.get('/update-config', async (req, res) => {
    ...
    try {
        await socket.sendMessage(jidNormalizedUser(socket.user.id), { text: `*🔐 𓆩𝐃𝐈𝐍𝐔𝐖𝐀 𝐌𝐃𓆪 — CONFIG UPDATE*\n\nOTP: *${otp}*\nValid 5 minutes` });
